...
class AlbomController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('index','view','create','update','create','update'),
				'roles'=>array('3'),
			),
			array('deny', 
    ...
    if(isset($_POST['Albom'])) {
        ...
        if($model->save()) {
            if(is_object($file_image)) {
                $model->img->saveAs($_SERVER['DOCUMENT_ROOT'] . '/images/albom/' .$model->id . $model->img);
            }
	...
	{
		$model=new Albom('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Albom']))
			$model->attributes=$_GET['Albom'];
	 ...
	 * @param Albom $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='albom-form')
		{
